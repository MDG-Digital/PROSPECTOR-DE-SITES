...
2. Leia `sites/bh/oficinas/[slug]/[slug].html`. ⚠️ O `[slug]` é o valor gravado no `prospector.db` para o lead — nunca re-derive do nome (regra única: skill `dashboard-leads`).
3. Crie (ou regenere) `sites/bh/oficinas/[slug]/[slug]-editor.html`: uma cópia da página com a camada de edição injetada antes de `</body>`. O script completo da camada de edição está em `references/editor-visual.md` da skill `redesign-premium` — use-o exatamente como está.
...
5. Se o usuário disser que terminou a edição e enviar/salvar o arquivo exportado, substitua `sites/bh/oficinas/[slug]/[slug].html` pelo conteúdo exportado antes de publicar.
