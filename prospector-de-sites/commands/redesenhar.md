...
3. **Salvar** na pasta conectada. ⚠️ **O `[slug]` é o valor JÁ GRAVADO no `prospector.db` para este lead (case pelo `nome`) — leia-o do banco e use EXATAMENTE; nunca re-derive do nome aqui** (regra única de slug: skill `dashboard-leads`). Arquivos:
   - `sites/bh/oficinas/[slug]/[slug].html` — a página final (arquivo único, autocontido, responsivo)
   - `sites/bh/oficinas/[slug]/[slug]-editor.html` — a MESMA página com a camada de edição visual injetada antes de `</body>` (script completo em `references/editor-visual.md` da skill `redesign-premium`). Gere SEMPRE, sem esperar o usuário pedir.
...
- [ ] `sites/bh/oficinas/[slug]/[slug].html` para CADA cliente do lote
- [ ] `sites/bh/oficinas/[slug]/[slug]-editor.html` para CADA cliente do lote
- [ ] **Slug conferido:** para CADA cliente, o nome da pasta `sites/bh/oficinas/<slug>/` é IDÊNTICO ao `slug` do lead no `prospector.db`. Se divergir, corrija para o slug do banco ANTES de prosseguir — pasta/arquivo com nome diferente do banco quebra os botões PÁGINA/EDITAR SITE do dashboard.
