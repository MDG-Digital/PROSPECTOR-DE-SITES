...
O `slug` é a **identidade** do lead: é o mesmo nome usado para a pasta (`sites/bh/oficinas/<slug>/`), os arquivos (`<slug>.html`, `<slug>-editor.html`, `proposta.html`, `contrato-<slug>...`) e a URL pública (`.../<pastaBase>/<slug>/`). Se o slug do banco divergir do nome da pasta/arquivo, os botões do dashboard quebram. Por isso:
...
3. **Checagem antes de criar/publicar arquivos:** confirme que `sites/bh/oficinas/<slug>/` usa o mesmo `slug` do banco. Se a pasta não existir com esse nome mas existir com outro, é sinal de divergência — pare e alinhe pelo slug do banco antes de continuar.
