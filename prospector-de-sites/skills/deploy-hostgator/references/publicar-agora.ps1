...
if (-not (Test-Path "fila-publicacao.txt")) {
	if (-not $Auto) {
		Log "Nada na fila - peca /publicar ao Claude primeiro." "Yellow"
	}
	Fim 0
}
...
$u = $cfg.hostgator.usuario
$p = $cfg.hostgator.senha
$srv = $cfg.hostgator.servidor

if (-not $u -or -not $p -or -not $srv) {
	Log "ERRO: preencha a conexao HostGator (dashboard > Configuracoes) incluindo a senha." "Red"
	Fim 1
}
