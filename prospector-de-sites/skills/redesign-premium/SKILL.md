...
5. **Arquivo único + slug do banco.** `sites/[slug]/[slug].html` autocontido: CSS inline no `<head>`, sem build, sem dependências além de Google Fonts. **O `[slug]` NÃO é derivado do nome aqui — leia o slug já gravado no `prospector.db` para este lead (case pelo `nome`) e use-o EXATAMENTE** na pasta e nos arquivos (`<slug>.html`, `<slug>-editor.html`). Regra única de slug: skill `dashboard-leads`. É PROIBIDO inventar um slug novo — é isso que faz pasta/arquivo divergirem do banco e quebra os botões do dashboard.
