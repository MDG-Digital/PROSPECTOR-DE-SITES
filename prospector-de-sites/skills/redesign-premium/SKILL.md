...
description: Esta skill deve ser usada ao redesenhar o site de um cliente prospectado OU criar a primeira página de um cliente que ainda não tem site — versão premium e de alta conversão, mantendo conteúdo, logo e paleta quando existirem, e usando Instagram e Google como fonte quando não houver site. Acione quando o usuário disser "redesenhar site", "melhorar página", "refazer o site do cliente", "criar site do cliente" ou rodar /redesenhar ou /editor.
---

# Redesign premium de páginas

Criar uma NOVA VERSÃO da página do cliente — não uma página nova. O cliente precisa reconhecer o próprio negócio, só que elevado ao padrão que o faturamento dele merece. **Quando o cliente ainda NÃO tem site**, criar a PRIMEIRA página a partir das informações públicas reais (Google Meu Negócio, avaliações do Google e Instagram) — mesmas regras de "nada inventado".

## Regras invioláveis

1. **Nenhum FATO inventado — mas o texto deve ser APRIMORADO.** Todo serviço, credencial, número, endereço e contato vem do site original quando existir, ou — inclusive quando há site — do perfil do Google (Maps/GMN) e do Instagram do cliente. Sem dados fictícios, sem depoimentos criados, sem serviços que o cliente não oferece. Porém o TEXTO não é copiado cru: reescreva com copy melhor — títulos mais fortes, frases mais claras, hierarquia de leitura — sempre dizendo a mesma verdade que o original diz.
2. **Fotos e logo originais são OBRIGATÓRIOS no site novo.** Toda foto utilizável do site existente (profissional, consultório, logo) deve constar na página nova, pelas URLs originais (colete via `img.currentSrc` no navegador, rolando a página inteira para vencer lazy-load). O cliente precisa se reconhecer na hora. **Se o cliente não tem site**, use as fotos públicas do Instagram e do Google Meu Negócio (fachada, equipe, trabalhos) e o logo do perfil; sem logo utilizável, use composição tipográfica — nunca invente logo.
...
5. **Arquivo único + slug do banco.** `sites/bh/oficinas/[slug]/[slug].html` autocontido: CSS inline no `<head>`, sem build, sem dependências além de Google Fonts. **O `[slug]` NÃO é derivado do nome aqui — leia o slug já gravado no `prospector.db` para este lead (case pelo `nome`) e use-o EXATAMENTE** na pasta e nos arquivos (`<slug>.html`, `<slug>-editor.html`). Regra única de slug: skill `dashboard-leads`. É PROIBIDO inventar um slug novo — é isso que faz pasta/arquivo divergirem do banco e quebra os botões do dashboard.
...
7. **Editor sempre.** Todo redesign gera junto o `sites/bh/oficinas/[slug]/[slug]-editor.html` (camada de edição de `references/editor-visual.md`) — nunca entregar página sem a versão editável.
